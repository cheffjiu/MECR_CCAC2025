...
class DefaultPromptConstructor(PromptConstructor):
    ...
    def _get_prompt_from_similar_samples(
        ...
        for i, item in enumerate(similar_samples, 1):
            ...
            stimulus_text = json.dumps(
                rationale["stimulus"]["textual"], ensure_ascii=False
            )
            stimulus_visual = (
                json.dumps(rationale["stimulus"]["visual"], ensure_ascii=False)
                if rationale["stimulus"]["visual"]
                else "null"
            )

            appraisal = json.dumps(rationale["appraisal"], ensure_ascii=False)
            response = json.dumps(rationale["response"], ensure_ascii=False)

            # 拼接完整格式
            prompt_similar_samples += f'[示例样本{i}]\n{dialogue_history}\n情感归因：\n{{\n  "rationale": {{\n    "stimulus": {{\n      "textual": {stimulus_text},\n      "visual": {stimulus_visual}\n    }},\n    "appraisal": {appraisal},\n    "response": {response}\n  }}\n}}\n\n'

...
sys.path.append(os.path.join(project_root, "GNN_model"))
...
from GNN_model.build_emotion_graph import build_emotion_graph
from GNN_model.GAT_modal import EmotionGraphEncoder
from GNN_model.collate_fn import collate_to_graph_batch
...
class RealDataFusionDataset(FusionFeatureDataset):
    def __getitem__(self, idx):
        ...
        mapping_path = os.path.join(
            self.feature_root, sid, "text_feature/id_mapping.json"
        )
...
def real_data_test():
    ...
    dataset = RealDataFusionDataset(
        feature_root=config["feature_root"], json_path=config["json_path"], mode="train"
    )

    # 2. 实例化适配模型
    fusion_model = RealDataCrossModalAttention().to(config["device"])
    gat_model = EmotionGraphEncoder(in_dim=512).to(
        config["device"]
    )  # 输入维度匹配融合输出
...
if __name__ == "__main__":
    ...
    sample_path = os.path.join(
        config["feature_root"], "anjia_sample1/text_feature/anjia_sample1.pt"
    )
    text_feat = torch.load(sample_path)
    print(f"文本特征实际维度: {text_feat.shape} (应为 [5,768])")

    video_path = os.path.join(
        config["feature_root"], "anjia_sample1/video_feature/anjia_sample1_video.pt"
    )
